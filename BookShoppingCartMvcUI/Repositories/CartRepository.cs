...
namespace DrugShoppingCartMvcUI.Repositories
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<int> AddItem(int drugId, int qty)
        {
            ...
            try
            {
                ...
                var cartItem = _db.CartDetails
                                  .FirstOrDefault(a => a.ShoppingCartId == cart.Id && a.DrugId == drugId);
                ...
                {
                    var drug = _db.Drugs.Find(drugId);
                    cartItem = new CartDetail
                    {
                        DrugId = drugId,
                        ShoppingCartId = cart.Id,
                        Quantity = qty,
                        UnitPrice = drug.Price  // it is a new line after update
                    };
        ...
        public async Task<int> RemoveItem(int drugId)
        {
            //using var transaction = _db.Database.BeginTransaction();
            string userId = GetUserId();
            try
        ...
        public async Task<bool> DoCheckout(CheckoutModel model)
        {
            ...
            try
            {
                ...
                foreach(var item in cartDetail)
                {
                    ...
                    var stock = await _db.Stocks.FirstOrDefaultAsync(a => a.DrugId == item.DrugId);
                    if (stock == null)
                    {
